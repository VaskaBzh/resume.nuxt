...
watchEffect(() => {
  if (props.activeOptions) {
    if (props.multipleType) {
      const activeOptionNames = props.activeOptions.map(option => option.name)
      localOptions.value = localOptions.value.filter(option => !activeOptionNames.includes(option.name))
      localActiveOptions.value = props.activeOptions
    } else if (props.activeOptions.length > 0) {
      const activeOptionName = props.activeOptions[0].name
      localOptions.value = localOptions.value.filter(option => option.name !== activeOptionName)
...
const changeActiveOption = (item: OptionsInterface): void => {
  if (!props.multipleType) {
    ...
    openMenu()
    // localOptions.value.splice(localOptionsIndex, 1)
    // if (oldItem) { localOptions.value.unshift(oldItem) }
  } else {
    const localOptionsIndex = localOptions.value.findIndex(option => option.name === item.name)
    ...
    emit('change', { new_item: toRaw(item) })
